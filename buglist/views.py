...
from bugapp.models import Item, List, Comment
from bugapp.forms import AddListForm, AddItemForm, EditItemForm, AddExternalItemForm, SearchForm
from buglist import settings
...
def list_lists(request):
    ...
    return render_to_response('buglist/list_lists.html', locals(), context_instance=RequestContext(request))
...
def del_list(request,list_id,list_slug):
    ...
    """                                                                         
    Delete an entire list. Danger Will Robinson! Only staff members should be a\llowed to access this view.                                                     
    """
    ...
    return render_to_response('buglist/del_list.html', locals(), context_instance=RequestContext(request))
...
def view_list(request,list_id=0,list_slug=None,view_completed=0):
        ...
        if form.is_valid():
            ...
            if "notify" in request.POST :
                if new_task.assigned_to != request.user :
                    ...
                    # Send email                                                
                    email_subject = render_to_string("buglist/email/assigned_subject.txt", { 'task': new_task })
                    email_body = render_to_string("buglist/email/assigned_body.txt", { 'task': new_task, 'site': current_site, })
                    try:
    ...
    return render_to_response('buglist/view_list.html', locals(), context_instance=RequestContext(request))
...
def view_task(request,task_id):
        ...
        if request.POST:
             ...
             if form.is_valid():
                 ...
                 if request.POST['comment-body']:
                     ...
                     email_subject = render_to_string("buglist/email/assigned_subject.txt", { 'task': task })                    
                     email_body = render_to_string("buglist/email/newcomment_body.txt", { 'task': task, 'body':request.POST['comment-body'], 'site': current_site, 'user':request.user })
                 ...
                 return HttpResponseRedirect(reverse('buglist-incomplete_tasks', args=[task.list.id, task.list.slug]))
    ...
    return render_to_response('buglist/view_task.html', locals(), context_instance=RequestContext(request))
...
def external_add(request):
    ...
    This is useful if, for example, a core web team are in a group that can file buglists for each other,
    ...
    if request.POST:
            ...
            if form.is_valid():
                ...
                # Send email
                email_subject = render_to_string("buglist/email/assigned_subject.txt", { 'task': item.title })                    
                email_body = render_to_string("buglist/email/assigned_body.txt", { 'task': item, 'site': current_site, })
                try:
    ...
    return render_to_response('/Users/Michael/Projects/django-bug-master/buglist/templates/buglist/add_external_task.html', locals(), context_instance=RequestContext(request))
...
def add_list(request):
    """
    Allow users to add a new buglist list to the group they're in.
    """
    ...
    return render_to_response('buglist/add_list.html', locals(), context_instance=RequestContext(request))
...
def search(request):
    ...
    return render_to_response('buglist/search_results.html',
